...
public class BookServiceExceptionHandler {
    ...
    public ResponseEntity<VndErrors> bookNotFoundError(BookNotFoundException e, WebRequest request) {
    ...
    @ResponseStatus(value = HttpStatus.UNPROCESSABLE_ENTITY)
    public ResponseEntity<VndErrors> noteNotFoundError(NoteNotFoundException e, WebRequest request) {
        VndErrors error = new VndErrors(request.toString(), e.getMessage());
        ResponseEntity<VndErrors> responseEntity = new ResponseEntity<>(error, HttpStatus.UNPROCESSABLE_ENTITY);
        return responseEntity;
    }
